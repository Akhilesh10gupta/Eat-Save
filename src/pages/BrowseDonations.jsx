...
const BrowseDonations = () => {
  ...
  const handleSearch = async () => {
    try {
      ...
      if (filters.foodName) query.append("foodName", filters.foodName);
      if (filters.location) query.append("location", filters.location);
      if (filters.isFree) query.append("isFree", filters.isFree);
      if (filters.status) query.append("status", filters.status);

      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/browse/donations/search?${query.toString()}`,
        {
          headers: {
            "EXTRABITE-API-KEY": import.meta.env.VITE_API_KEY,
          },
            ...
            {loading ? (
              ...
              donations.map((item, index) => (
                ...
                >
                  ...
                  <p className="text-sm">Free: <span className="font-semibold">{item.isFree ? "Yes" : "No"}</span></p>
                  {!item.isFree && (
                    <p className="text-sm">Price: <span className="font-semibold">₹{item.price}</span></p>
                  )}
                  <p className="text-sm">Location: <span className="font-semibold">{item.location}</span></p>
                  <p className="text-sm">Delivery: <span className="font-semibold">{item.deliveryType}</span></p>
                  <p className="text-sm">Donor: <span className="font-semibold">{item.donorName}</span></p>
                </div>
              ))
            )}
          </div>
